...
exports.createJarvisScriptFromPath = createJarvisScriptFromPath;
const promises_1 = require("node:fs/promises");
const node_path_1 = require("node:path");
async function createJarvisScriptFromPath(sourcePath, scope, siteId) {
...
async function readJarvisScriptManifest(sourcePath) {
    const sourceStat = await (0, promises_1.stat)(sourcePath).catch(() => undefined);
    if (!sourceStat?.isDirectory()) {
        throw new Error("请选择 Jarvis Script 目录");
    }
    const manifestPath = (0, node_path_1.join)(sourcePath, "jarvis-script.json");
    const rawText = await (0, promises_1.readFile)(manifestPath, "utf8").catch((error) => {
...
function readMonitor(value, index) {
...
function readWorker(value, index) {
    if (!value || typeof value !== "object") {
...
function readContentScript(value, index) {
    if (!value || typeof value !== "object") {
...
function stablePathId(path) {
    const source = (0, node_path_1.basename)(path) || path;
    let hash = 0;
